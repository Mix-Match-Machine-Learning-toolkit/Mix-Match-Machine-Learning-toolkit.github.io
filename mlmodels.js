...
function loadInteractiveExample(path) {
  ...
  xhttpCombi.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      document.getElementById("interactiveExample").innerHTML =
        this.responseText;
      if (path == "tableClassification.html") {
        document.getElementById("rs-range-line1").addEventListener(
          "input",
          function () {
            showSliderValue("rs-range-line1", "rs-bullet1");
          },
          false
        );
        document.getElementById("rs-range-line2").addEventListener(
          "input",
          function () {
            showSliderValue("rs-range-line2", "rs-bullet2");
          },
          false
        );
        document.getElementById("rs-range-line3").addEventListener(
          "input",
          function () {
            showSliderValue("rs-range-line3", "rs-bullet3");
          },
          false
        );
        document.getElementById("rs-range-line4").addEventListener(
          "input",
          function () {
            showSliderValue("rs-range-line4", "rs-bullet4");
          },
          false
        );
      }
...
async function textClassification() {
  // console.log(textClassModel);
  await tf.ready();
  await tf.setBackend("webgl"); // or use 'cpu' if webgl causes issues
  ...
  document.getElementById("probInsult").innerHTML =
    (probabilityInsult * 100).toFixed(2) + "%";
  document.getElementById("probToxicity").innerHTML =
    (probabilityToxic * 100).toFixed(2) + "%";
}
...
async function classifyImage() {
  ...
  console.log(
    document.getElementById("label2").offsetHeight,
    document.getElementById("prob2").offsetHeight
  );

  document.getElementById("prob1").style.height =
    document.getElementById("label1").offsetHeight + "px";
  document.getElementById("prob2").style.height =
    document.getElementById("label2").offsetHeight + "px";
  document.getElementById("prob3").style.height =
    document.getElementById("label3").offsetHeight + "px";
  console.log(
    document.getElementById("label2").offsetHeight,
    document.getElementById("prob2").offsetHeight
  );
...
ml5
  .styleTransfer("assets/MLmodels/wave")
...
ml5
  .styleTransfer("assets/MLmodels/udnie")
...
/// Declare recognizer in the global scope using var
var recognizer;

async function startListening() {
  try {
    console.log("Starting tf.ready() and setting backend...");
    // Ensure TensorFlow.js is fully loaded and ready
    await tf.ready();
    await tf.setBackend("webgl"); // or use 'cpu' if webgl causes issues

    console.log("TensorFlow ready. Checking recognizer initialization...");

    // Initialize recognizer only if it hasn’t been initialized yet
    if (!recognizer) {
      console.log("Initializing recognizer...");
      recognizer = speechCommands.create("BROWSER_FFT");
      await recognizer.ensureModelLoaded();
      console.log("Recognizer initialized and model loaded.");
    } else {
      console.log("Recognizer is already initialized.");
    }
    ...
    console.error("Error during recognizer initialization:", error);
  }
}

function predictWord() {
  // Ensure recognizer is defined before proceeding
  if (!recognizer) {
    console.warn("predictWord called before recognizer was initialized");
    return;
  }

  console.log("Starting word prediction...");
  ...
  recognizer.listen(
    ({ scores }) => {
      ...
      // Sort scores in descending order to get the top predictions
      scores.sort((s1, s2) => s2.score - s1.score);

      // Display the top 3 predictions
      document.querySelector("#wordPredicted1").textContent = scores[0].word;
      document.querySelector("#probWord1").textContent =
        (scores[0].score * 100).toFixed(2) + "%";
      document.querySelector("#wordPredicted2").textContent = scores[1].word;
      document.querySelector("#probWord2").textContent =
        (scores[1].score * 100).toFixed(2) + "%";
      document.querySelector("#wordPredicted3").textContent = scores[2].word;
      document.querySelector("#probWord3").textContent =
        (scores[2].score * 100).toFixed(2) + "%";
    },
...
function stopListening() {
  // Check if recognizer is defined and listening
...
async function classifyPenguins() {
  ...
  tf.tidy(() => {
    document.getElementById("buttonClassify").innerHTML = "Classifying";
    tf.loadLayersModel("/assets/MLmodels/palmerpenguins/model.json").then(
      (model) => {
        //scaled features
        let scaled_bill_length =
          (bill_length - 44.19227468) / 5.261841782685142;
        let scaled_bill_depth = (bill_depth - 17.13690987) / 1.997404512327992;
        let scaled_flipper_length =
          (flipper_length - 201.40343348) / 13.343284828475145;
        let scaled_body_mass = (body_mass - 4218.24034335) / 795.7032642413537;

        const scaled_features = tf.tensor1d([
          scaled_bill_length,
          scaled_bill_depth,
          scaled_flipper_length,
          scaled_body_mass,
        ]);
        ...
        // const prob3 = prob[2];
        document.getElementById("penguinImage").src =
          "assets/photos/" + penguinClasses[winners[0]] + ".png";
        document.getElementById("predictions").style.display = "block";
        document.getElementById("label1").innerHTML =
          penguinClasses[winners[0]];
        document.getElementById("prob1").innerHTML =
          (prob[0] * 100).toFixed(2) + "%";
        document.getElementById("label2").innerHTML =
          penguinClasses[winners[1]];
        document.getElementById("prob2").innerHTML =
          (prob[1] * 100).toFixed(2) + "%";
        document.getElementById("label3").innerHTML =
          penguinClasses[winners[2]];
        document.getElementById("prob3").innerHTML =
...
function recommendMovie() {
  ...
  switch (movie) {
    case "The Dark Knight Rises":
      ...
      document
        .getElementById("movieRecommendations")
        .appendChild(printMovies(recTheDarkKnightRises));
      break;
    case "The Intouchables":
      console.log(recTheIntouchables);
      document
        .getElementById("movieRecommendations")
        .appendChild(printMovies(recTheIntouchables));
      break;
    case "The Lion King":
      console.log(recTheLionKing);
      document
        .getElementById("movieRecommendations")
        .appendChild(printMovies(recTheLionKing));
      break;
    case "Schindler's List":
      console.log(recSchindlersList);
      document
        .getElementById("movieRecommendations")
        .appendChild(printMovies(recSchindlersList));
      break;
    default:
      console.warn("No recommendations found for the selected movie.");
...
function printMovies(recommendedMovies) {
  ...
  if (!recommendedMovies) {
    console.warn("No movies to recommend.");
    return document.createTextNode("No recommendations available.");
  }
